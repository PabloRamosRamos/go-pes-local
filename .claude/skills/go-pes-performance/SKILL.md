...
9. **Evita el segundo round-trip tras guardar.** Un guardado suele venir seguido de un re-fetch del módulo para re-render — **otra ejecución GAS con su propio warmup (~1,5-3 s)**. En su lugar: extrae un **builder único** (DRY) compartido por el endpoint de **lectura** y el de **guardado**; el guardado **devuelve la vista ya actualizada** (`res.avance`/`res.detalle`) y el frontend renderiza desde ahí, con **fallback** al re-fetch si no viene. Para una **lista**, devuelve la **fila** actualizada (misma forma que la lista) y **parchéala en el estado del cliente** (con re-orden), sin re-pedir todo. Precedentes (2026-09): `goPesBuildAvanceOrganizacionPayload_`, `goPesBuildAvanceGrupoVecinosPayload_`, `goPesBuildOrganizacionModuloDetalle_`, `goPesBuildSocioModuloRow_`. **Excepción:** paneles que reconstruyen **todas** las filas con **sincronización que escribe** (Beneficios/CÁMARAS: `buildCamaras1414Panel_` llama `syncAllCamaras1414Eligibility_`) **no encajan** — devolverlos metería el costo+escritura en el guardado; ahí el fix es abaratar el build/sync, no el round-trip.
10. **Poda telemetría redundante en guardados.** Un `logProcessing_('INFO', …, 'OK', …)` que **duplica** el `logUserAction_` de la misma acción cuesta ~0,4-0,6 s por guardado (append a `LOG_Procesamiento`). Quítalo — la auditoría queda en `LOG_Acciones_Usuario`. **Conserva** siempre WARN/ERROR/CRÍTICO (se monitorean) y los resultados `PARCIAL` (detalle de fallos de importación).
