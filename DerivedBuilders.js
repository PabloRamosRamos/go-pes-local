...
function refreshPartialArtifacts_(options) {
  ...
  const config = Object.assign({
    masterSolicitudIds: [],
    vistaOrganizacionIds: [],
    vistaInstrumentoIds: [],
    vistaTerritorialPairs: [],
    sugerenciaSolicitudIds: [],
    sugerenciaOrganizacionIds: [],
  ...
  uniqueNonBlank_(config.masterSolicitudIds).forEach(function(solicitudId) {
    upsertMasterRowBySolicitudId_(solicitudId);
  });

  uniqueNonBlank_(config.vistaOrganizacionIds).forEach(function(organizacionId) {
    upsertVistaOrganizacionRowById_(organizacionId);
  });

  uniqueNonBlank_(config.vistaInstrumentoIds).forEach(function(orgInstrumentoId) {
    upsertVistaInstrumentoRowById_(orgInstrumentoId);
  });

  uniqueCompositePairs_(config.vistaTerritorialPairs).forEach(function(pair) {
    upsertVistaTerritorialRow_(pair.uv, pair.sector);
  });

  uniqueNonBlank_(config.sugerenciaSolicitudIds).forEach(function(solicitudId) {
    upsertSuggestionRowsForSolicitudId_(solicitudId);
  });

  uniqueNonBlank_(config.sugerenciaOrganizacionIds).forEach(function(organizacionId) {
    upsertSuggestionRowForOrganizacionId_(organizacionId);
  });

  uniqueCompositePairs_(config.territorioCatalogPairs).forEach(function(pair) {
    upsertTerritorioCatalogRow_(pair.uv, pair.sector);
  });

  uniqueNonBlank_(config.responsables).forEach(function(nombre) {
    upsertResponsableCatalogRow_(nombre);
  });
...
function uniqueCompositePairs_(pairs) {
  ...
  (pairs || []).forEach(function(pair) {
    const uv = String(pair && pair.uv || '').trim();
    const sector = String(pair && pair.sector || '').trim();
    if (!uv && !sector) return;

    const key = uv + '||' + sector;
    if (seen[key]) return;
    seen[key] = true;
    out.push({ uv: uv, sector: sector });
  });

  return out;
}

function getInstrumentoActivoStates_() {
  return [
    'identificado',
    'en evaluacion',
    'en habilitacion',
    'habilitado',
    'postulacion en preparacion',
    'postulacion enviada',
    'en revision',
    'adjudicado',
    'en implementacion'
...
function upsertCompositeRow_(sheetName, headers, keyFields, record) {
  if (!record) return;

  const sh = ensureSheetWithHeaders_(sheetName, headers);
  const data = getSheetData_(sheetName);
  const targetKey = (keyFields || []).map(function(field) {
  ...
  for (let i = 0; i < data.length; i++) {
    const currentKey = (keyFields || []).map(function(field) {
      return String(data[i][field] || '').trim();
    }).join('||');
...
function findOrganizacionForSolicitud_(solicitudId, preferredOrgId) {
  const S = GO_PES_V2.SHEETS;
  if (preferredOrgId) {
    const preferred = findByField_(S.MAE_ORGANIZACIONES, 'organizacion_id', preferredOrgId, false);
    if (preferred) return preferred;
  }

  const rows = filterByField_(S.MAE_ORGANIZACIONES, 'solicitud_id', solicitudId, false) || [];
  return rows.length ? rows[0] : null;
}

function buildMasterRecordBySolicitudId_(solicitudId) {
  const S = GO_PES_V2.SHEETS;
  const c = findByField_(S.MAE_CASOS, 'solicitud_id', solicitudId, false);
  if (!c) return null;

  const org = findOrganizacionForSolicitud_(solicitudId, c.organizacion_id || '');
  const orgId = (org && org.organizacion_id) || c.organizacion_id || '';
  const instrumentos = orgId ? filterByField_(S.FACT_INSTRUMENTOS, 'organizacion_id', orgId, false) : [];
  const requisitos = orgId ? filterByField_(S.FACT_REQUISITOS, 'organizacion_id', orgId, false) : [];
  const socios = orgId ? filterByField_(S.FACT_SOCIOS, 'organizacion_id', orgId, false) : [];
  const acciones = sheetExists_(S.LOG_ACCIONES)
    ? getSheetData_(S.LOG_ACCIONES).filter(function(r) {
        return String(r.entity_id || '') === String(solicitudId || '') ||
          String(r.entity_id || '') === String(orgId || '');
      })
    : [];

  const activoStates = getInstrumentoActivoStates_();
  const cantidadInstrumentosActivos = instrumentos.filter(function(r) {
    return activoStates.indexOf(normalizeText_(r.estado_instrumento)) !== -1;
  }).length;

  const cantidadRequisitosPendientes = requisitos.filter(function(r) {
    return ['pendiente', 'solicitado', 'rechazado', 'vencido'].indexOf(normalizeText_(r.estado_requisito)) !== -1;
  }).length;

  let lastAction = {};
  acciones.forEach(function(r) {
    const currentTs = new Date(r.timestamp || 0).getTime();
    const previousTs = new Date(lastAction.timestamp || 0).getTime();
    if (!lastAction.timestamp || currentTs >= previousTs) {
      lastAction = r;
    }
  });
  ...
  return {
    ...
    uv: (org && org.uv) || c.uv || '',
    sector: (org && org.sector) || c.sector || '',
    fecha_ingreso: c.fecha_ingreso || '',
    estado_actual: c.estado_actual || '',
    etapa_actual: c.etapa_actual || '',
    proximo_hito: c.proximo_hito || '',
    responsable_actual: (org && org.responsable_actual) || c.responsable_actual || '',
    nombre_organizacion: (org && org.nombre_organizacion) || '',
    estado_constitucion: (org && org.estado_constitucion) || '',
    estado_general_organizacion: (org && org.estado_general_organizacion) || '',
    cantidad_instrumentos_activos: cantidadInstrumentosActivos,
    cantidad_requisitos_pendientes: cantidadRequisitosPendientes,
    cantidad_socios: socios.length,
    ultimo_usuario: lastAction.email || '',
    ultima_actualizacion: lastAction.timestamp || c.updated_at || (org && org.updated_at) || ''
  };
}

function upsertMasterRowBySolicitudId_(solicitudId) {
  const S = GO_PES_V2.SHEETS;
  const record = buildMasterRecordBySolicitudId_(solicitudId);
  if (!record) return;
  upsertByKey_(S.MASTER, 'solicitud_id', record, false);
}

function buildVistaOrganizacionRecordById_(organizacionId) {
  const S = GO_PES_V2.SHEETS;
  const org = findByField_(S.MAE_ORGANIZACIONES, 'organizacion_id', organizacionId, false);
  if (!org) return null;

  const instrumentos = filterByField_(S.FACT_INSTRUMENTOS, 'organizacion_id', organizacionId, false);
  const socios = filterByField_(S.FACT_SOCIOS, 'organizacion_id', organizacionId, false);
  ...
  return {
    ...
    cantidad_instrumentos_activos: instrumentos.length,
    cantidad_socios: socios.length,
...
function upsertVistaOrganizacionRowById_(organizacionId) {
  const S = GO_PES_V2.SHEETS;
  const record = buildVistaOrganizacionRecordById_(organizacionId);
  if (!record) return;
  upsertByKey_(S.VW_ORGS, 'organizacion_id', record, false);
}

function buildVistaInstrumentoRecordById_(orgInstrumentoId) {
  const S = GO_PES_V2.SHEETS;
  const item = findByField_(S.FACT_INSTRUMENTOS, 'org_instrumento_id', orgInstrumentoId, false);
  if (!item) return null;

  const org = findByField_(S.MAE_ORGANIZACIONES, 'organizacion_id', item.organizacion_id || '', false) || {};
  return {
    org_instrumento_id: item.org_instrumento_id || '',
    organizacion_id: item.organizacion_id || '',
    nombre_organizacion: org.nombre_organizacion || '',
    instrumento_codigo_catalogo: item.instrumento_codigo_catalogo || '',
    instrumento_tipo: item.instrumento_tipo || '',
    origen_instrumento: item.origen_instrumento || '',
    estado_instrumento: item.estado_instrumento || '',
    resultado_instrumento: item.resultado_instrumento || '',
    avance_instrumento_pct: item.avance_instrumento_pct || '',
    responsable_instrumento: item.responsable_instrumento || '',
    updated_at: item.updated_at || ''
  };
}

function upsertVistaInstrumentoRowById_(orgInstrumentoId) {
  const S = GO_PES_V2.SHEETS;
  const record = buildVistaInstrumentoRecordById_(orgInstrumentoId);
  if (!record) return;
  upsertByKey_(S.VW_INSTR, 'org_instrumento_id', record, false);
}

function buildSuggestionVecinoRecord_(caseRow) {
  if (!caseRow || !String(caseRow.vecino_id || '').trim()) return null;

  return {
    vecino_id: caseRow.vecino_id || '',
    solicitud_id: caseRow.solicitud_id || '',
    nombre_completo: caseRow.nombre_completo || buildFullName_(caseRow.nombre_vecino, caseRow.apellido_vecino) || '',
    telefono_contacto: caseRow.telefono_contacto || '',
    direccion_original: caseRow.direccion_original || '',
    uv: caseRow.uv || '',
    sector: caseRow.sector || '',
    estado_actual: caseRow.estado_actual || '',
    organizacion_id: caseRow.organizacion_id || '',
    updated_at: caseRow.updated_at || caseRow.ultima_gestion || caseRow.fecha_ingreso || ''
  };
}

function buildSuggestionSolicitudRecord_(caseRow) {
  if (!caseRow || !String(caseRow.solicitud_id || '').trim()) return null;
  ...
  return {
    ...
    estado_actual: caseRow.estado_actual || '',
    organizacion_id: caseRow.organizacion_id || '',
    fecha_ingreso: caseRow.fecha_ingreso || '',
    updated_at: caseRow.updated_at || caseRow.ultima_gestion || ''
  };
}

function buildSuggestionOrganizacionRecord_(orgRow) {
  if (!orgRow || !String(orgRow.organizacion_id || '').trim()) return null;

  return {
    organizacion_id: orgRow.organizacion_id || '',
    nombre_organizacion: orgRow.nombre_organizacion || '',
    tipo_organizacion: orgRow.tipo_organizacion || '',
    uv: orgRow.uv || '',
    sector: orgRow.sector || '',
    estado_constitucion: orgRow.estado_constitucion || '',
    estado_general_organizacion: orgRow.estado_general_organizacion || '',
    responsable_actual: orgRow.responsable_actual || '',
    updated_at: orgRow.updated_at || ''
  };
}

function upsertSuggestionRowsForSolicitudId_(solicitudId) {
  const S = GO_PES_V2.SHEETS;
  const caseRow = findByField_(S.MAE_CASOS, 'solicitud_id', solicitudId, false);
  if (!caseRow) return;

  const vecRecord = buildSuggestionVecinoRecord_(caseRow);
  const solRecord = buildSuggestionSolicitudRecord_(caseRow);
  const orgRow = findOrganizacionForSolicitud_(solicitudId, caseRow.organizacion_id || '');
  const orgRecord = buildSuggestionOrganizacionRecord_(orgRow);

  if (vecRecord) upsertByKey_(S.DIM_VEC_SUG, 'vecino_id', vecRecord, false);
  if (solRecord) upsertByKey_(S.DIM_SOL_SUG, 'solicitud_id', solRecord, false);
  if (orgRecord) upsertByKey_(S.DIM_ORG_SUG, 'organizacion_id', orgRecord, false);
}

function upsertSuggestionRowForOrganizacionId_(organizacionId) {
  const S = GO_PES_V2.SHEETS;
  const orgRow = findByField_(S.MAE_ORGANIZACIONES, 'organizacion_id', organizacionId, false);
  const orgRecord = buildSuggestionOrganizacionRecord_(orgRow);
  if (orgRecord) upsertByKey_(S.DIM_ORG_SUG, 'organizacion_id', orgRecord, false);
}

function buildVistaTerritorialRecord_(uv, sector) {
  const S = GO_PES_V2.SHEETS;
  const uvKey = String(uv || '').trim();
  const sectorKey = String(sector || '').trim();
  if (!uvKey && !sectorKey) return null;

  const casos = getSheetData_(S.MAE_CASOS).filter(function(r) {
    return String(r.uv || '').trim() === uvKey && String(r.sector || '').trim() === sectorKey;
  });

  const orgs = getSheetData_(S.MAE_ORGANIZACIONES).filter(function(r) {
    return String(r.uv || '').trim() === uvKey && String(r.sector || '').trim() === sectorKey;
  });

  const orgById = {};
  orgs.forEach(function(r) {
    orgById[String(r.organizacion_id || '').trim()] = r;
  });

  const activoStates = getInstrumentoActivoStates_();
  const instrumentosActivos = getSheetData_(S.FACT_INSTRUMENTOS).filter(function(r) {
    const orgId = String(r.organizacion_id || '').trim();
    if (!orgById[orgId]) return false;
    return activoStates.indexOf(normalizeText_(r.estado_instrumento)) !== -1;
  }).length;
  ...
  return {
    uv: uvKey,
    sector: sectorKey,
    total_solicitudes: casos.length,
    total_organizaciones: orgs.length,
    organizaciones_constituidas: orgs.filter(function(r) {
      return normalizeText_(r.estado_constitucion) === 'constituida';
    }).length,
    instrumentos_activos: instrumentosActivos
  };
}

function upsertVistaTerritorialRow_(uv, sector) {
  const S = GO_PES_V2.SHEETS;
  const headers = buildSheetDefinitions_()[S.VW_TERR];
  const record = buildVistaTerritorialRecord_(uv, sector);
  if (!record) return;
  upsertCompositeRow_(S.VW_TERR, headers, ['uv', 'sector'], record);
}

function upsertTerritorioCatalogRow_(uv, sector) {
  const S = GO_PES_V2.SHEETS;
  const uvKey = String(uv || '').trim();
  const sectorKey = String(sector || '').trim();
  if (!uvKey && !sectorKey) return;

  upsertCompositeRow_(
    S.DIM_TERRITORIO,
    buildSheetDefinitions_()[S.DIM_TERRITORIO],
    ['uv', 'sector'],
    {
      uv: uvKey,
      sector: sectorKey,
...
function upsertResponsableCatalogRow_(nombreResponsable) {
  const S = GO_PES_V2.SHEETS;
  const nombre = String(nombreResponsable || '').trim();
  if (!nombre) return;

  upsertByKey_(S.DIM_RESPONSABLES, 'nombre_responsable', {
    responsable_id: nextIdIfMissing_('responsable', 'RSP', S.DIM_RESPONSABLES, 'responsable_id', 'nombre_responsable', nombre),
