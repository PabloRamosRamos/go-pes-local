...
| Métrica | Valor |
|---------|-------|
| **Archivos de código** | 43 (21 JS + 22 HTML) |
...
- [ ] **Corrección de gestiones de caso** (feature nueva) — hoy `guardarSeguimiento` es append-only; no se puede corregir una gestión ya registrada. Una gestión es una fila editable de `FACT_Hitos`, así que **encaja en `aplicarCorreccionAuditada_`**: falta backend (`editarGestion`) + UI (botón editar en la sección de gestiones de la ficha, `Scripts_Ficha.html`). Ver skill `go-pes-correccion`.
- [ ] **Corrección de inscripciones a eventos** (feature nueva) — `goPesUpsertFormInscripcion` es create-only; corregir un RUT/nombre mal tecleado hoy exige cancelar y recrear. Falta un flujo de edición in-place auditado (encaja en la central).

## Historial de cambios significativos

- **2026-08-27 (Correcciones — Migración de los flujos de edición existentes a la función central auditada)** — Se incorporó la corrección auditada (antes→después en `LOG_Acciones_Usuario`) a los flujos que YA permitían editar datos, en orden de criticidad, uno por uno y verificando en DEV. **Migrados a `aplicarCorreccionAuditada_`** (log del *antes→después*, `LockService`, log-o-aborta): `editarDatosVecino` (además **ampliado** para corregir nombre/apellido/RUT del vecino —antes solo contacto—, con recálculo de `nombre_completo` y refresco de sugerencias; frontend en `Scripts_Ficha.html`), `editarDatosSocio`, `actualizarCargoSocioOrganizacion` (`UPDATE_SOCIO_CARGO`; además ganó lock que no tenía), `cambiarEstadoAdministrativoOrganizacion_` (suspender/reactivar, **grado 2**: `requireMotivo` + confirmación que nombra la consecuencia en `Scripts_Organizaciones.html`), `guardarOrganizacion`/`guardarInstrumento`/`guardarRequisito` (se **separó alta de corrección**: `CREATE_*` vs `EDIT_*`; el alta usa `upsert` directo, la edición pasa por la central; se quitó el `LockService` externo para no anidar con el de la central), `goPesUpsertFondese` (rama edición → `EDIT_FONDESE` vía **`applyFn`** que envuelve `upsertRowsByKey_` + sync a `FACT_Instrumentos`, para que la reversión restaure también el ledger; la regla de adjudicación `goPesAssertAdjudicacionFondese_` NO cambió; alta → `CREATE_FONDESE`), `goPesUpsertFormEvento` (rama edición → `EDIT_FORM_EVENTO`; **fix latente**: la edición borraba `created_by`/`created_at`, ahora la central los preserva; alta → `CREATE_FORM_EVENTO`). **Renombrado por consistencia** (create-only, sin corrección in-place): `goPesUpsertFormInscripcion` → `CREATE_FORM_INSCRIPCION`. **Casos que NO usan la central por diseño** (documentado como excepción en la skill): `cambiarEstadoAvance` (evento append-only → se enriqueció el log con `estado_anterior`) y `guardarCamaras1414Organizacion` (escritura acoplada multi-hoja con estado derivado → antes→después con `diffCamposCorreccion_` sin invocar la central, porque su reversión por-registro dejaría el detalle inconsistente). **Brechas registradas (sin vía de corrección in-place, requieren feature nueva):** gestiones de caso (`guardarSeguimiento`, append-only) e inscripciones a eventos (create-only). Skill `go-pes-correccion` actualizada (regla obligatoria + **cláusula de excepción** para eventos/escrituras acopladas). Batería `goPesRunAllTests()` verificada en DEV: **291 pasados, 0 fallados**. Archivos: `Services.js`, `ZZ_SociosBackend.js`, `ZZ_OrganizacionesBackend.js`, `ZZ_AvanceBackend.js`, `ZZ_BeneficiosBackend.js`, `Scripts_Ficha.html`, `Scripts_Organizaciones.html`, `skills/go-pes-correccion/**`. Deploy: solo DEV.

- **2026-08-26 (Correcciones — Función central de corrección auditada + skill `go-pes-correccion`)** — Se construyó la pieza transversal para que TODOS los flujos corrijan datos igual, en vez de reimplementar la lógica cada uno. **(1) Skill de proyecto `go-pes-correccion`** (`skills/` + espejo `.claude/skills/` + reglas bundleadas): impone que corregir = editar **y** registrar el *antes→después* en `LOG_Acciones_Usuario` (nunca sobrescritura silenciosa), trazabilidad **log-o-aborta** (si no se puede auditar, no se aplica), prevención (validación/confirmación/resumen antes de guardar), grados de corrección por impacto y cuidado de PII en logs. **Decisión del dueño (2026-08-26): el sistema NO trabaja con autoría de registros** — la reja es el acceso al módulo (`requireModuleAccess_`, + `requireRole_` donde el grado lo exija), no el autor. **(2) Pieza compartida `go-pes-local/Corrections.js`:** `aplicarCorreccionAuditada_(params)` (wrapper con `LockService` + `getUsuarioActual` + `findByField_` + `upsertByKey_`/`applyFn` + `logUserAction_`) y núcleo puro `aplicarCorreccionCore_` (diff solo-cambios, no-op si nada cambió, `requireMotivo` para grado 2, fija `updated_at`, **reversión al snapshot anterior si el log falla**, concurrencia optimista `expectedAntes`, `extraDetail` para contexto del log). Helpers `diffCamposCorreccion_`, `sanitizarCambiosPII_`/`enmascararPII_`, lista `GO_PES_PII_FIELDS_`. **(3) Migrados los dos flujos de edición manual** (antes logueaban solo el valor nuevo; ahora *antes→después* con PII enmascarada): `editarDatosSocio` (`ZZ_SociosBackend.js`; conserva la acción histórica `EDIT_SOCIO` y agrega `organizacion_id` vía `extraDetail`) y `editarDatosVecino` (`Services.js`; conserva `EDIT_VECINO_CONTACTO`; el `applyFn` por defecto `upsertByKey_` reproduce exactamente `patchCaseSummary_`; refresco de vistas solo si hubo cambios). Ambos sueltan su `LockService` propio (lo maneja la central, evita lock anidado). **Cambio de comportamiento intencional:** guardar sin modificar nada ya no escribe ni loguea (devuelve `sin_cambios:true`). **(4) Tests:** suite `goPesTestCorrecciones_` en `Audith.js` (20 tests: diff, PII, no-op, motivo, reversión log-o-aborta, concurrencia, `extraDetail`), registrada en `goPesRunAllTests`; batería 271→**291 activos**. El núcleo puro se validó además en local con Node (0 fallos). Archivos: `go-pes-local/Corrections.js` (nuevo), `ZZ_SociosBackend.js`, `Services.js`, `Audith.js`, `skills/go-pes-correccion/**`, `docs/dev-stats.md`. Deploy: solo DEV.

- **2026-08-20 (Beneficios/CAPACITACIONES — Vinculación a la ficha del vecino + fix PIN)** — CAPACITACIONES es un beneficio **por socio/persona** (no por org); se vinculó a la **ficha del vecino**. **Backend:** nuevo helper `getCapacitacionesByRut_(rut)` (`ZZ_BeneficiosBackend.js`) que une `FACT_Form_Inscripciones`+`FACT_Form_Eventos` por RUT (no canceladas); `obtenerFicha` (`Services.js`) ahora incluye `capacitaciones` en su respuesta (por `caseRow.rut_vecino`). **Frontend:** `renderFichaCapacitacionesSection_` en `Scripts_Ficha.html` renderiza la sección "Capacitaciones y formación" (tipo badge por color, título, fecha) tras el universo del vecino; CSS `ficha-cap-*` en `Styles.html` (tokens de tema). **Fix:** en `goPesUpsertFormEvento`, `actorEmail` se usaba en la validación del PIN de eventos abiertos antes de definirse (`undefined` al rate-limiting) — movida la definición arriba. Archivos: `ZZ_BeneficiosBackend.js`, `Services.js`, `Scripts_Ficha.html`, `Styles.html` + skill go-pes-beneficios. Deploy: solo DEV.

- **2026-08-19 (Beneficios/CÁMARAS 1414 — UI de tabla de seguimiento con acciones por fila)** — El panel CÁMARAS pasó de config + dos tablas + ficha de detalle a **una sola tabla de seguimiento** (opción "Con progreso"). `buildCamaras1414Panel_` reescrito para devolver `{orgs, summary}` (cada org: estado/estado_index, fecha_certificado, fecha_solicitud, fecha_convenio, camaras, plazo_tono/plazo_badge/plazo_mensaje); `buildCamarasPlazoInfo_` gana `badge`/`restantes`. **Frontend:** `renderCamaras1414Panel_` (hero + stat strip + tabla), `renderCamaras1414TrackingTable_`, `camProgress_` (stepper Elegible→Solicitado→Instalado), `camPlazoBadge_` (badge de color por plazo/resultado), `camActionsHtml_`/`camEditorHtml_` + `bindCamaras1414Panel_` con delegación: botón "Marcar solicitud" → editor inline de fecha; "Registrar instalación" → fecha + N° cámaras; llaman a `guardarCamaras1414Organizacion` con payload **parcial** (ahora **fusiona** solicitud/instalacion con lo existente para no borrar la otra). **Plazos ya no configurables** (config-form eliminado; plazo fijo 5 días hábiles). CSS `cam-*` en `injectFondeseStyles` (variables de tema). **Limpieza:** eliminadas `renderCamaras1414Table_`, `renderCamaras1414Detail_`, `collectCamaras1414DetailPayload_`, `validateCamaras1414DetailForm_`, `getCamaras1414EligibilityIssue_`, `renderBeneficiosMetric_`, `renderBeneficiosAlertCard_`, `beneficioAlertClass_`, `beneficioBadgeClass_` (frontend) y `buildCamarasDetailByOrgId_`, `buildCamarasAssignmentSummary_` (backend); `syncAllCamaras1414Eligibility_` reducida a solo asegurar elegibilidad. Archivos: `ZZ_BeneficiosBackend.js`, `Scripts_Beneficios.html` + skill. Deploy: solo DEV.

- **2026-08-19 (Beneficios/CÁMARAS 1414 — Simplificación al flujo real "solicitar y registrar")** — CÁMARAS resultó mucho más simple que lo implementado: beneficio **único por organización**, rol del equipo = solicitar y registrar. Rediseñado a **3 estados: Elegible → Solicitado → Instalado**. (1) Elegible al obtener el certificado definitivo (FOR_04); (2) **solicitar** la instalación a Seguridad Pública dentro de **5 días hábiles (lun-vie)** del hito; (3) al recibir la firma del convenio, **registrar** que las cámaras están instaladas y **cuántas**. **Backend** (`ZZ_BeneficiosBackend.js`): `buildCamarasWorkflowState_` reescrito (lee detail rows `SOLICITUD_SP`/`INSTALACION_REG`; se eliminó `resolveCamarasStageIndex_`), `getCamarasStateOptions_` (3), `guardarCamaras1414Organizacion` recibe `{solicitud, instalacion}`, `buildCamarasDetailRows_` (2 rows), `buildCamarasPlazoInfo_` + helpers `businessDaysBetween_`/`addBusinessDays_`, `buildCamarasAlertRows_` (check único de plazo hábil), `isCamaras{Eligible,Active}ListRow_` a los estados nuevos. **Frontend** (`Scripts_Beneficios.html`): `renderCamaras1414Detail_` reducido a 2 secciones (Solicitud con badge de plazo; Instalación con N° de cámaras + fecha de convenio); `collect`/`validate` simplificados; config-form recortado a un solo campo (días hábiles). **Alertas** (`Alertas.js`): `evaluarBenCamarasPostCert_` en días hábiles + chequeo real de solicitud. **Limpieza:** eliminadas ~28 funciones muertas de CÁMARAS (los `build*View_`, `normalize*Payload_` de checklist/correo/visita/convenio/cierre, `buildCamarasSingleDetailRow_`, `resolveCamarasPayload*`, `buildCamaras{Tracking,Inactivity}Summary_`, `getCamarasSociosByOrganizacionId_`, `getCamarasChecklistDefinitions_`, `sanitizeCamarasSelect_`; frontend `syncCamarasCommitteeContactUi_`/`copyTextToClipboard_`/`openCamaras1414Mailto_`). Tests de CÁMARAS reescritos (3 estados, workflow, días hábiles): suite Beneficios 32→16, total 273→257. Datos viejos de detail rows quedan huérfanos pero no se borran. Archivos: `ZZ_BeneficiosBackend.js`, `Scripts_Beneficios.html`, `Alertas.js`, `Audith.js`, docs (`dev-stats.md`) + skill go-pes-beneficios. Deploy: solo DEV.

- **2026-08-19 (Beneficios/FONDESE — Madurez: ledger, montos, modelo de estados por ventanas de acompañamiento y alertas)** — FONDESE llevado al nivel operativo de CÁMARAS, reencuadrado a la operación real del equipo PES (monitorear/registrar/alertar en dos ventanas de responsabilidad con un hueco de evaluación municipal en medio). **(0) Fix de pérdida de datos:** `goPesUpsertFondese` reconstruía la fila completa vía `upsertRowsByKey_` (que rellena con `''` las columnas ausentes) — los botones "Iniciar rendición"/"Cerrar proceso" mandan payload parcial y **borraban** org, convocatoria, línea, checklist, etc. Ahora **fusiona** el payload sobre la fila existente (leer→`Object.assign`→escribir). También el fallback oculto de `observaciones_rendicion` (forzaba `value=""`) preserva el valor. **(A) Modelo de estados:** ciclo `en_armado → ingresada → en_evaluacion → adjudicado → firma_convenio → en_ejecucion → en_rendicion → cerrado`. Se agregó `en_armado` (Ventana 1: PES acompaña el armado antes de ingresar) y se renombró `postulando`→`ingresada` (handoff, fuera de manos). `getFondeseStateOptions_`/`buildFondeseWorkflowState_` nuevos (espejo de CÁMARAS). Migración one-run `goPesMigrateFondeseEstados()`. Sincronizada la 3ª definición de headers de `FACT_Fondese` en `ZZ_AdminDataReset.js` (expectedCols 14→18, hubiera borrado columnas). **(Montos + ledger):** `FACT_Fondese` gana `monto_adjudicado`/`monto_ejecutado` (definidos en `Repository.buildSheetDefinitions_`, `goPesEnsureFondeseSheets_` y `ZZ_AdminDataReset.js`), capturables en el detalle **solo tras adjudicación**; `syncFactInstrumentoFromFondese_` refleja cada postulación en `FACT_Instrumentos` (clave determinista por `fondese_id`, idempotente) para que aparezca en la ficha de la org y el dashboard; `goPesBackfillFondeseInstrumentos()` para las existentes. **(B) Puerta de entrada (solo backend; UI al final):** `goPesGetFondeseHabilitadas(idEdicion)` (todas las orgs FOR_04 unidas a su estado FONDESE, o `habilitada`) y `goPesIngresarFondeseArmado(payload)` (valida FOR_04 + no-duplicado, crea `en_armado`). **(C) Línea editable** en el detalle solo mientras `en_armado`; bloqueada desde `ingresada`. **(D) Alertas FONDESE** (`Alertas.js`): `evaluarBenFondeseCierreConv_` (WARNING, Ventana 1: convocatoria por cerrar con armados sin ingresar) y `evaluarBenFondeseRendicion_` (DANGER, Ventana 2: rendición por vencer/vencida sin aprobar); umbrales `ben_fondese_cierre_conv_dias` (10)/`ben_fondese_rendicion_dias` (15); nada durante evaluación. Tests en `Audith.js` (IDs, umbrales, evaluadores). **(Llamados + adjudicación):** cada edición tiene dos llamados fijos (Primer/Segundo Llamado FONDESE 20XX, `goPesFondeseLlamadoNombre_`); una org puede postular a ambos (unicidad por llamado en `goPesIngresarFondeseArmado`, ya no por edición), y `goPesGetFondeseHabilitadas(idEdicion, convocatoriaId)` opera por llamado con banderas `adjudicada_este_anio`/`bloqueada_adjudicacion`. Nueva regla `goPesAssertAdjudicacionFondese_` en `goPesUpsertFondese`: una sola adjudicación por año calendario (ambos llamados) y solo si la anterior cerró con rendición aprobada. **(UI puerta de entrada, opción "tabla de monitoreo"):** el panel FONDESE abre en un listado de organizaciones habilitadas (`renderFondeseHabilitadasListado_`/`loadFondeseHabilitadas_`/`bindFondeseHabilitadasListado_`) con selector Año + selector Llamado en una fila, "Bases del llamado" colapsable (fechas/presupuesto/documentos/líneas con monto), stat strip y tabla agrupada "requieren ingreso al proceso"/"en proceso"; acción por org "Ingreso al proceso" (`openFondeseIngresarArmado_`) y "Abrir" → detalle. `goPesGetFondeseHabilitadas` devuelve además `monto_adjudicado`/`alerta` de plazo por fila. CSS `fnd-years/fnd-llamados/fnd-bases/fnd-hab-*` en `injectFondeseStyles` (variables de tema). Loaders estandarizados (`.module-loading` vía `fndLoadingHtml_`). Pestaña de beneficio activa: verde plano (`--brand-green`/`--brand-green-soft`) sin lift/sombra en `Styles.html`. **Limpieza:** eliminados `renderFondeseTable_` y `openFondeseForm_` (frontend) y `goPesGetFondeseList`/`goPesGetOrgsElegiblesFondese` (backend), sin llamadores tras el rediseño; skips de Audith actualizados a las funciones nuevas. Archivos: `ZZ_BeneficiosBackend.js`, `Repository.js`, `Alertas.js`, `Main.js`, `ZZ_AdminDataReset.js`, `Scripts_Beneficios.html`, `Styles.html`, `Audith.js`, docs (`api-interna.md`, `modelo-datos.md`, `alertas.md`) + skill go-pes-beneficios. Deploy: solo DEV.

- **2026-07-14 (Organizaciones — Toolbar rediseñado + fix hito Cert. provisorio + filtro Abandonaron)** — **(1) Fix de dato:** la stat card "Cert. provisorio" contaba el hito 9 (que en la numeración del módulo es "Cuenta pública"); corregida al **hito 7** (PRE_07), consistente con los labels del formulario inline de fechas. **(2) Layout:** nueva fila `.org-toolbar` de dos columnas — búsqueda + chips de filtro a la izquierda (donde estaban las cards), stat cards compactas alineadas a la derecha; colapsa a una columna bajo 1100px. Las stat cards y el chip-row migraron de CSS inline a clases (`.org-stat-card*`, `.org-chip-row`, `.org-stats-grid`) en Styles.html con variables de tema (dark se resuelve solo). **(3) Nuevo chip "Abandonaron (N)":** filtra elementos que abandonaron el proceso según `esAbandonoProceso_()` — estado de avance vigente "Detenido" (último registro de FACT_Avance_Estado, expuesto como `estado_avance` por `getOrganizacionesConGruposClient` usando los índices `buildEstadosBy*Index_`), organización suspendida/inactiva, o caso del grupo en "No continúa"/"Desistido" (`estado_actual` nuevo en items de grupo). Archivos: `ZZ_OrganizacionesBackend.js`, `Scripts_Organizaciones.html`, `Styles.html`. Deploy: solo DEV.

- **2026-07-14 (Fix — Ingreso de vecino desde calendario: 6 quiebres corregidos)** — Revisión completa del flujo Calendario → Nuevo Ingreso → Ficha. **(1) Cruce calendario↔hitos muerto:** `getCalendarioEventos` leía `GO_PES_V2.AVANCE.FACT_HITOS` (clave inexistente → hoja `undefined` → `[]` silencioso) y filtraba por `h.hito_key` (campo inexistente; la hoja define `codigo_hito`) — `yaIngresado`/`hitoInfo` jamás se poblaban y los eventos ya ingresados seguían apareciendo como pendientes. Corregido a `SHEETS.FACT_AVANCE_HITOS` + `codigo_hito`; `hitoInfo.responsable` corregido de `responsable_gestion` a `usuario_registro`. **(2) Mismo bug en Alertas.js:** las 5 evaluaciones de formalización filtraban por `hito_key` — las alertas nunca han disparado en PROD; corregido a `codigo_hito` (validar contra datos reales al pasar a PROD). **(3) Registro de eventos procesados per-user:** `registrarEventoCalendario` guardaba en UserProperties — un evento registrado por el operador A seguía pendiente para B (riesgo de ingreso duplicado). Migrado a ScriptProperties (compartido) con lectura de transición del store per-user antiguo y poda a 60 días (el fetch solo mira 14 días atrás; tope ~9KB por clave). El mock DEV ahora aplica el mismo filtro para que el flujo sea verificable. **(4) Estado fantasma `calendarioPendingEvent`:** solo se limpiaba tras guardado exitoso — un ingreso manual posterior arrastraba el hito PRE_01 y la marca del evento de otro vecino. Ahora se descarta al navegar fuera de Nuevo Ingreso (`route()`), al limpiar el formulario, y hay banner visible (`#ingreso-cal-banner`, CSS en Styles.html con variables de tema) con botón Desvincular. **(5) Evento visible tras guardar:** el camino "Realizado" no filtraba el evento de `APP.state.calendarioData` ni avisaba si `registrarEventoCalendario` fallaba (catch vacío) — ahora filtra la lista local y muestra error operativo con re-fetch forzado si el marcado falla. **(6) Carrera de visibilidad post-guardado (candidata del error 0 s de `obtenerFicha`):** `guardarIngreso` hace `SpreadsheetApp.flush()` antes de liberar el lock, y `obtenerFicha` reintenta una vez (sleep 500 ms + invalidación de cache) si el `solicitud_id` no aparece. También eliminado un `route('ficha')` redundante post-`loadFicha` en el diálogo de coincidencias del calendario (ocultaba el loader con la ficha en vuelo). Archivos: `ZZ_CalendarioBackend.js`, `Alertas.js`, `Services.js`, `Scripts_NuevoIngreso.html`, `Scripts_Calendario.html`, `Scripts.html`, `Index.html`, `Styles.html`. Deploy: solo DEV.

- **2026-07-14 (Documentación — Reglas anti-regresión)** — Los vicios encontrados en las auditorías 2026-07 quedaron codificados como reglas obligatorias para no repetirlos. `CLAUDE.md`: nuevas secciones "Higiene de código — anti-duplicados y anti-código muerto" (grep antes de crear funciones por el namespace único de GAS/IIFE, prohibido desactivar en vez de eliminar, migraciones completas sin alias permanentes, verificación de clases CSS en ambos sentidos, scan de duplicados al cerrar cambios) y "Performance y datos — reglas aprendidas" (`GO_PES_RUNTIME` muere por ejecución → cache persistente solo con `CacheService`; no enganchar invalidaciones a rutas que los logs recorren; writes solo vía helpers de Repository; prohibidos los tiempos fijos; guard anti doble envío en acciones > 2 s; solo las 3 formas estándar de loader; endpoints compuestos para el arranque). `mantener-docs.md`: anti-patrones documentales (docs completados → archive en el mismo cambio, no crear docs paralelos del mismo tema, árbol de CLAUDE.md al día, grep en docs al eliminar APIs, pendientes marcados al cerrarse). `inicio-rapido-claude.md`: resumen de ambas para nuevas sesiones. Archivos: `docs/CLAUDE.md`, `docs/mantener-docs.md`, `docs/inicio-rapido-claude.md`.
...
### Higiene de código — anti-duplicados y anti-código muerto

Reglas nacidas de la auditoría 2026-07-13 (se encontraron 3 colisiones de nombre, ~430 líneas muertas y CSS huérfano):

1. **Antes de crear una función, grep del nombre exacto en todo el proyecto.** El backend GAS comparte UN solo namespace global (todos los `.js`) y los parciales `Scripts_*.html` comparten UN solo IIFE: dos funciones con el mismo nombre no dan error — la última definición **pisa silenciosamente** a la primera. Este vicio produjo un bug real (la `invalidateRequestIndexes_` activa no invalidaba hitos/socios).
2. **Nunca "arreglar" redefiniendo.** Si una función existe y necesita cambios, edita la original. Prohibido pegar una versión nueva más abajo o en otro archivo.
3. **El código muerto se elimina, no se desactiva.** Prohibido dejar funciones con `return;` al inicio, sufijos `*LegacyInactive_`, bloques comentados "por si acaso" o markup sin llamadores. Git conserva la historia; si algo debe sobrevivir temporalmente, márcalo `@deprecated` con fecha de eliminación y regístralo en "Tareas pendientes".
4. **Al eliminar o migrar una API, migra TODAS las llamadas y borra la vieja en el mismo cambio.** Los alias "de compatibilidad" solo se justifican mientras dura la migración, nunca como estado final.
5. **CSS: al crear una clase verifica que el JS/HTML la use, y al usar una clase verifica que exista en `Styles.html`.** Se encontró `.spinner` usada sin definición (loader invisible) y `.view-loader` definida sin ningún uso.
6. **Verificación al cerrar cualquier cambio:** `grep` de los símbolos eliminados (debe dar vacío) y scan de duplicados (`grep -hoE '^function \w+' *.js | sort | uniq -d` y equivalente en parciales).

### Performance y datos — reglas aprendidas (2026-07)

1. **`GO_PES_RUNTIME` muere con cada ejecución.** Cada `google.script.run` es una ejecución GAS nueva con memoria global vacía: un cache con TTL guardado en `GO_PES_RUNTIME` NUNCA acierta entre llamadas (solo dentro de la misma). Cache persistente = `CacheService.getScriptCache()` (compartido) o `.getUserCache()` (por usuario), payload JSON < 90 KB, TTL en segundos, con invalidación explícita en los `invalidate*_()` correspondientes. Patrón de referencia: `Catalogs.js` y `Dashboard.js`/`Alertas.js`.
2. **No enganchar invalidación de caches a `invalidateRequestIndexes_` ni a los appends genéricos**: los logs también escriben hojas — el cache se invalidaría en cada acción y nunca viviría.
3. **Writes siempre vía helpers de `Repository.js`** (`appendRowObject_`, `upsertByKey_`, etc.). Estos ya memoizan la verificación de hoja (`ensuredSheets`) y los headers (`headersBySheet`) por ejecución — no agregar verificaciones ni relecturas de headers por write. Los writes invalidan con `invalidateSheetDataCache_` (solo datos); `invalidateSheetRuntimeCache_` (completa) es solo para cambios estructurales de hoja.
4. **Prohibidos los tiempos fijos para enmascarar latencia** (`setTimeout` mágicos, duraciones mínimas de splash/loaders). La UI reacciona al estado real: se muestra al iniciar el trabajo y se oculta cuando termina. La única animación de cierre permitida es la transición del design system (~220 ms).
5. **Toda acción que tarde > 2 s lleva guard anti doble envío** (botón deshabilitado o flag en `APP.state` liberado en `finally`). El overlay del modal NO basta: ESC permite cerrar y reintentar. La validación de duplicados en backend es red de seguridad, no la primera línea.
6. **Loaders: solo las 3 formas estándar** (módulo / modal / local) documentadas en `design-system.md` → "Sistema de Loading". Prohibido crear sistemas de loading propios por módulo o loaders `position:fixed`.
7. **Endpoints de arranque: minimizar round-trips.** Si una vista necesita N datasets al abrir, expón UN endpoint compuesto (patrón `getInicioBootstrapData()`), no N llamadas `google.script.run` en paralelo — cada una paga ejecución completa con su auth.
