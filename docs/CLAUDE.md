...
- **2026-07-15 (Avance — Rediseño visual: card unificada av2)** — El módulo Avance reemplaza el layout de "Ficha de avance + barras de resumen + dos grillas de hitos por tramo" por una sola card (`av2-card`): header con avatar de iniciales, nombre, chips de estado (Activo/Stand by/Detenido/Finalizado con punto de color; chip extra "Grupo de vecinos" cuando aplica, chip de solicitud) y anillo SVG de progreso N/total; barras segmentadas por tramo (flex 7/8, un segmento por hito: cumplido verde `#8CC63F`, actual tinte+borde primario, bloqueado neutro; candado en el label de Formalización si el tramo está bloqueado); sección colapsable de hitos cumplidos (role=button, aria-expanded, transición max-height con `prefers-reduced-motion`; fecha y usuario por hito cruzando el timeline por `codigo_hito`); hero del hito actual (círculo con orden, badge HITO ACTUAL, descripción del catálogo, "Disponible desde"/"Requiere: hito previo ✓", CTA `Registrar hito` → `registrarHitoAvanceUi_`; hitos disponibles adicionales como filas compactas; estados "Proceso completado", "Sin hitos disponibles" y "Proceso {estado}" con motivo cuando no está Activo, con CTA deshabilitado); footer con siguiente hito bloqueado + usuario del último hito. **Funciones:** `renderAvanceResumen_` reescrita (genera toda la card), `renderAvanceBotones_` pasa a ser el wiring post-render (rebind necesario porque el innerHTML se regenera en cada carga); eliminadas sin llamadores `renderAvanceProgressRow_`, `buildProgressBarHtml_`, `clampPct_`; las secciones de grillas salieron del template y de `clearAvanceView_`. **CSS:** bloque nuevo `<style id="avance-v2-styles">` en `Index.html` (prefijo `av2-`, decisión de scope: Styles.html y ThemeDark.html no se tocaron); tintes con `color-mix` sobre variables de tema (claro/oscuro se resuelve solo, un único override dark local para el check verde). **Deuda registrada:** clases `avance-hitos-grid`/`avance-hito-card`/`avance-resumen-*` quedaron muertas en `Styles.html` (limpieza pendiente en sesión de Styles); los botones `avance-standby-btn`/`avance-detener-btn`/`avance-reactivar-btn` y `avance-estado-badge` siguen sin renderizarse en ningún template (resto pre-rediseño: `cambiarEstadoAvanceUi_` es inalcanzable desde la UI; los guards null-safe se conservan). Selectores de búsqueda, modal de hito, modal recordatorio de beneficio, timeline y APIs sin cambios. Archivos: `Scripts_Avance.html`, `Index.html`. Deploy: solo DEV.
